...
# Function to create the optimizer for training
...
# train one epoch
async def train(segmentation_module, iterator, optimizers, history, epoch, args, noti_bot):
    ...
    for i in range(args.epoch_iters):
        ...
        if not args.num_gpus > 1:
            ...
            # GPU 사용 가능 확인 후, 배치 단위로 GPU로 데이터 이동
        ...
        if i % args.disp_iter == 0:
            ...
            print_msg = 'Id: {}\n Epoch: [{}][{}/{}], Time: {:.2f}, Data: {:.2f}, LR: encoder {:.6f}, decoder {:.6f}, {}, {}' \
                .format(args.id, epoch, i, args.epoch_iters,
                        batch_time.average(), data_time.average(),
                        args.running_lr_encoder, args.running_lr_decoder,
                        acc_info, loss_info)
            print(print_msg)
            if (i % args.epoch_iters == 0):
                async with noti_bot:
                    await noti_bot.sendMessage(chat_id='-976984844', text=print_msg)

            wandb.log({"Epoch": epoch, "Iteration": i, "Time": batch_time.average(), "Data-Time": data_time.average(),
                    "Encoder LR": args.running_lr_encoder, "Decoder LR": args.running_lr_decoder,
                    "Object Accuracy": ave_metric['object'].average(), "Part Accuracy": ave_metric['part'].average(),
                    "Material Accuracy": ave_metric['material'].average(), "Scene Accuracy": ave_metric['scene'].average(),
                    "Object Loss": ave_losses['object'].average(), "Part Loss": ave_losses['part'].average(),
                    "Material Loss": ave_losses['material'].average(), "Scene Loss": ave_losses['scene'].average(), })

            # wandb.log({"Epoch": epoch, "Iteration": i, "Time": batch_time.average(), "Data-Time": data_time.average(),
            #         "Encoder LR": args.running_lr_encoder, "Decoder LR": args.running_lr_decoder,
            #         "Part Accuracy": ave_metric['part'].average(),
            #         "Part Loss": ave_losses['part'].average(),
            #         })
...
async def main(args):
    ...
    # for param in net_decoder.conv_fusion.parameters():
    #     param.requires_grad = True
    for param in net_decoder.scene_head.parameters():
        param.requires_grad = True
    for param in net_decoder.object_head.parameters():
        param.requires_grad = True
    for param in net_decoder.part_head.parameters():
        param.requires_grad = True
    for param in net_decoder.material_head.parameters():
        param.requires_grad = True
    ...
    if args.num_gpus > 1:
        segmentation_module = UserScatteredDataParallel(
            segmentation_module, device_ids=range(args.num_gpus))
        # For sync bn
        patch_replication_callback(segmentation_module)
    #segmentation_module = nn.DataParallel(segmentation_module, device_ids=range(args.num_gpus))
    ...
    noti_bot = telegram.Bot(token=args.noti_token)

    for epoch in range(args.start_epoch, args.num_epoch + 1):
        await train(segmentation_module, iterator_train, optimizers, history, epoch, args, noti_bot)
...
if __name__ == '__main__':
    assert version_parse(torch.__version__) >= version_parse('0.4.0'), \
        'PyTorch>=0.4.0 is required'

    parser = argparse.ArgumentParser()
    # Model related arguments
    parser.add_argument('--id', default='se_ulsam',
                        help="a name for identifying the model")
    parser.add_argument('--arch_encoder', default='resnet50',
                        help="architecture of net_encoder")
    parser.add_argument('--arch_decoder', default='upernet',
                        help="architecture of net_decoder")
    parser.add_argument('--weights_encoder', default='upp-resnet50-upernet/encoder_epoch_40.pth',
                        help="weights to finetune net_encoder")
    parser.add_argument('--weights_decoder', default='upp-resnet50-upernet/decoder_epoch_40.pth',
    ...
    parser.add_argument('--lr_decoder', default=2e-3, type=float, help='LR')
    ...
    parser.add_argument('--noti_token', default='6229002937:AAE0VtSj8JjGH6Tl2ne9AmuNfAPHDmJ4VnA',
                        help='token')
    parser.add_argument('--noti_chatid', default='-976984844',
                        help='chatid')
    ...
    wandb.init(
        entity="soyeonpaper",
        project="upernet-soyeon",

        config={
            "arch_encoder": args.arch_encoder,
            "arch_decoder": args.arch_decoder,
            "num_gpus": args.num_gpus,
            "batch_size": args.batch_size,
            "imgMaxSize": args.imgMaxSize,
            "padding_constant": args.padding_constant,
            "segm_downsampling_rate": args.segm_downsampling_rate,
            "lr_encoder": args.lr_encoder,
            "lr_decoder": args.lr_decoder,
            "num_epoch": args.num_epoch,
            "weight_decay": args.weight_decay,
            "args.fix_bn": args.fix_bn,
            "ckpt": args.ckpt,
        }
    )
