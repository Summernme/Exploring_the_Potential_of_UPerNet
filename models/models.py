...
class UPerNet(nn.Module):
    def __init__(self, nr_classes, fc_dim=4096,
        ...
        self.se_block = SEBlock(in_channels=attn_dim)
        self.ulsam = ULSAM(nin=ul_dim, num_splits=4)
        self.head_in = nn.Conv2d(ul_dim*2, ul_dim, kernel_size=1)
        ...
        # input: PPM out, input_dim: attn_dim
        self.scene_head = nn.Sequential(
            conv3x3_bn_relu(attn_dim, attn_dim, 1),
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(attn_dim, self.nr_scene_class, kernel_size=1, bias=True)
        )

        # input: Fusion out, input_dim: attn_dim
        self.object_head = nn.Sequential(
            nn.Conv2d(ul_dim, ul_dim//2, 1),
            nn.Conv2d(ul_dim//2, attn_dim, 1),
            conv3x3_bn_relu(attn_dim, attn_dim, 1),
            nn.Conv2d(attn_dim, self.nr_object_class, kernel_size=1, bias=True)
        )
        # input: Fusion out, input_dim: attn_dim
        self.part_head = nn.Sequential(
            ...
            nn.Conv2d(ul_dim//2, attn_dim, 1),
            conv3x3_bn_relu(attn_dim, attn_dim, 1),
            nn.Conv2d(attn_dim, self.nr_part_class, kernel_size=1, bias=True)
        )

        # input: FPN_2 (P2), input_dim: attn_dim
        self.material_head = nn.Sequential(
            nn.Conv2d(ul_dim, ul_dim//2, 1),
            nn.Conv2d(ul_dim//2, attn_dim, 1),
            conv3x3_bn_relu(attn_dim, attn_dim, 1),
            nn.Conv2d(attn_dim, self.nr_material_class, kernel_size=1, bias=True)
        )
        ...
        if output_switch['object'] or output_switch['part'] or output_switch['material']:
            ...
            for i in range(1, len(fpn_feature_list)):
                fusion_list.append(F.interpolate(
                    ...
                    output_size,
                    mode='bilinear', align_corners=False))
            fpn_d1 = torch.cat((fusion_list[2], fusion_list[3]),1) #45
            fpn_d2 = torch.cat((fusion_list[0], fusion_list[3]),1) #25
            # x = self.conv_fusion(fusion_out)
            d1 = self.ulsam(fpn_d1)
            d2 = self.ulsam(fpn_d2)
            # d1 = self.da_block_d1(fpn_d1) * self.d1_w
            # d2 = self.da_block_d2(fpn_d2) * self.d2_w
            head_in = torch.cat((d1, d2), 1)
            # head_in = nn.Conv2d(head_in, head_in//2, kernel_size=1)
            head_in = self.head_in(head_in)


            if output_switch['scene']: # scene
                # x = self.se_block(s_att)
                output_dict['scene'] = self.scene_head(s_att)
            if output_switch['object']: # object
                output_dict['object'] = self.object_head(head_in)
            if output_switch['part']: # part
                output_dict['part'] = self.part_head(head_in)
            if output_switch['material']: # material
                output_dict['material'] = self.material_head(head_in)
