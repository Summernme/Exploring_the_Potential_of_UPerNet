...
def imresize(img, shape, interp=Image.NEAREST):
    return np.array(Image.fromarray(img).resize((shape[1], shape[0]), interp))
...
def uint16_imresize(seg, shape):
    return decodeRG(imresize(encodeRG(seg), shape, Image.NEAREST))
...
class TrainDataset(torchdata.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            # img
            img = imresize(img, (batch_resized_size[i, 0], batch_resized_size[i, 1]), Image.BILINEAR)
            ...
            if valid_obj:
                ...
                for j in range(broden_dataset.nr_object_with_part):
                    parts_resized.append(imresize(
                        seg_part[j], (batch_resized_size[i, 0], batch_resized_size[i, 1]), Image.NEAREST))
                for j in range(broden_dataset.nr_object_with_part):
                    if not valid_part[j]:
                        continue
                    ...
                    part = imresize(part_rounded,
                                    (part_rounded.shape[0] // self.segm_downsampling_rate,
                                     part_rounded.shape[1] // self.segm_downsampling_rate), Image.NEAREST)
            ...
            if valid_mat:
                ...
                segm = imresize(seg_material,
                                (batch_resized_size[i, 0], batch_resized_size[i, 1]), Image.NEAREST)
                segm_rounded_height = round2nearest_multiple(segm.shape[0], self.padding_constant)
                segm_rounded_width = round2nearest_multiple(segm.shape[1], self.padding_constant)
                ...
                segm = imresize(segm_rounded,
                                (segm_rounded.shape[0] // self.segm_downsampling_rate,
                                 segm_rounded.shape[1] // self.segm_downsampling_rate), Image.NEAREST)
